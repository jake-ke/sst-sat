...
void SATSolver::execAnalyze() {
    ...
    round_max_bt = -1;
    // mc-gmc1dl: capture every worker's learnt clause; winner_idx is the
...
void SATSolver::execBacktrack() {
    ...
    // mc-gmc1dl: bt-gated multi-commit. Extras are added ONLY when the round's
    ...
    // mc-gmc1dl: commit only the single best extra (min LBD, then min length)
    ...
    if (round_max_bt >= bt_level + 2) {
        ...
        for (size_t i = 0; i < round_learnts_raw.size(); i++) {
            ...
            if (round_learnts_raw[i].size() < 2) continue;
            if (round_lbds[i] > learnt_lbd) continue;
            if (best < 0
                || round_lbds[i] < round_lbds[best]
                || (round_lbds[i] == round_lbds[best]
                    && round_learnts_raw[i].size() < round_learnts_raw[best].size()))
                best = (int)i;
        }
        if (best >= 0) {
...
void SATSolver::analyze(Cref conflict, int worker_id) {
    ...
    // mc-gmc1dl: snapshot every worker's learnt clause (for adding to DB
