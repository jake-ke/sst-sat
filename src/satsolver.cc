...
void SATSolver::execAnalyze() {
    ...
    round_max_bt = -1;
    // mc-gmc1l: capture every worker's learnt clause; winner_idx is the
...
void SATSolver::execBacktrack() {
    ...
    // mc-gmc1l: bt-gated multi-commit. Extras are added ONLY when the round's
    ...
    // mc-gmc1l: commit only the single best extra (min LBD, then min length)
...
void SATSolver::analyze(Cref conflict, int worker_id) {
    ...
    // mc-gmc1l: snapshot every worker's learnt clause (for adding to DB
