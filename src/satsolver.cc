...
void SATSolver::execAnalyze() {
    ...
    round_max_bt = -1;
    // mc-gmc1: capture every worker's learnt clause; winner_idx is the
...
void SATSolver::execBacktrack() {
    ...
    // mc-gmc1: bt-gated multi-commit. Extras are added ONLY when the round's
...
void SATSolver::analyze(Cref conflict, int worker_id) {
    ...
    // mc-gmc1: snapshot every worker's learnt clause (for adding to DB
