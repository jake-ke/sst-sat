...
void SATSolver::execAnalyze() {
    ...
    round_max_bt = -1;
    // mc-lenmin: len-min selection (with bt tiebreak); bumpall accumulation.
    selected_len = std::numeric_limits<int>::max();
    v_to_bump.clear();
    c_to_bump.clear();
...
void SATSolver::analyze(Cref conflict, int worker_id) {
    ...
    // mc-lenmin: bumpall — always accumulate bumps from every worker.
    ...
    // mc-lenmin selection: shortest clause, bt-level tiebreak.
    int this_len = (int)tmp_learnt.size();
    if (this_len < selected_len
        || (this_len == selected_len && tmp_btlevel < bt_level)) {
        selected_len = this_len;
