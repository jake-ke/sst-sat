...
void SATSolver::execAnalyze() {
    ...
    round_max_bt = -1;
    // mc-gmcdl: capture every worker's learnt clause; winner_idx is the
...
void SATSolver::execBacktrack() {
    ...
    // mc-gmcdl: bt-gated multi-commit. Extras are added ONLY when the round's
    ...
    // mc-gmcdl: extras must be at least as good (LBD) as the winner —
...
void SATSolver::analyze(Cref conflict, int worker_id) {
    ...
    // mc-gmcdl: snapshot every worker's learnt clause (for adding to DB
